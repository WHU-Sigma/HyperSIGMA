...
def group_conv(in_channels, out_channels, kernel_size, groups, stride=1, bias=True):
    return nn.Conv2d(
...
class conv_sep(nn.Module):
    def __init__(self, in_channels, out_channels, ks, stride=1, dilation=1):
        super(conv_sep, self).__init__()
        self.deepwise = default_conv(in_channels, in_channels, kernel_size=ks, stride=stride, dilation=dilation, groups=in_channels)
        self.pointwise = default_conv(in_channels, out_channels, kernel_size=1, stride=1, dilation=1, groups=1)

    def forward(self, input):
        deep_result = self.deepwise(input)
        point_result = self.pointwise(deep_result)
...
class CSA(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        x1 = self.bn1(self.conv1(x)).view(b, c, -1)
        x2 = self.bn2(self.conv2(x)).view(b, c, -1)
        x3 = self.conv3(x).view(b, c, -1)
        CC = self.softmax(torch.bmm(x1, x2.permute(0,2,1)))
        out = torch.bmm(CC, x3).view(b, c, h, w)
        return x + self.a * out
