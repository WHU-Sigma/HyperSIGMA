...
class Evaluator(object):
    ...
    def Kappa_coefficient(self):
        ...
        num_total = np.sum(self.confusion_matrix, dtype=np.float64)
        observed_accuracy = np.trace(self.confusion_matrix, dtype=np.float64) / num_total
        expected_accuracy = np.sum(
            np.sum(self.confusion_matrix, axis=0, dtype=np.float64) / num_total * np.sum(self.confusion_matrix, axis=1,
                                                                                         dtype=np.float64) / num_total)
