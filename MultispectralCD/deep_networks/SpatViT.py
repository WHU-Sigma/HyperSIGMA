...
# alteration
# 1) SpatViT.init_weights(self, pretrained)函数中的num_extra_tokens = 1 改成num_extra_tokens = 0
# 2) patch_size == 8时的fpn1修改为最大池化kernel=2,stride=2.
class ALter_SpatViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
        ...
        if patch_size == 16:
            ...
            self.fpn4 = nn.Sequential(
                nn.MaxPool2d(kernel_size=2, stride=2),  # for 输入img_size = 16；patch_size =8；deform_inputs_func中整除8，
                # self.fpn4 = nn.Sequential(
                #     nn.MaxPool2d(kernel_size=4, stride=4),  # defalut
            )
