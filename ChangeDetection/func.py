...
def get_args(dataset,seed_i,use_checkpoint=True,mode='Spat_Spec_Pretraining'):
    ...
    parser.add_argument('--patch_size', type=int, default=patch_size, help='train_number')
    parser.add_argument('--seg_patches', type=int, default=seg_patches, help='number of patches')
    ...
    parser.add_argument('--channels', type=int, default=channels, help='train_number')
    parser.add_argument('--use_checkpoint', type=bool, default=use_checkpoint, help='')

    NUM_TOKENS = 144
    parser.add_argument('--mode', type=str, default=mode, help='') # True
    if mode =='Spat_Pretraining':
        parser.add_argument('--pretrain_path', type=str,
                            default='/data/meiqi.hu/PycharmProjects/Bigmodel/data/spat-vit-base-ultra-checkpoint-1599.pth',
                            help='')
        parser.add_argument('--interval', type=int, default=3, help='')  # True
        ...
        parser.add_argument('--pretrain_Specpath', type=str,
                            default='/data/meiqi.hu/PycharmProjects/Bigmodel/data/spec-vit-base-ultra-checkpoint-1599.pth',
                            help='')
        parser.add_argument('--interval', type=int, default=3, help='')  # True
        parser.add_argument('--embed_dim', type=int, default=768, help='train_number')  # 768
        parser.add_argument('--NUM_TOKENS', type=int, default=NUM_TOKENS, help='train_number')  # 768,64,
    else:
        raise NameError


    parser.add_argument('--seed', type=int, default=seed_i, help='number of seed')
    ...
    print('patch_size:', args.patch_size, ' | seg_patches:', args.seg_patches)  # ,' | proj:', args.proj
    print('embed_dim:', args.embed_dim, ' | channels:', args.channels)  # ,' | proj:', args.proj
...
def get_Traindata(args):
    setup_seed(seed=1)
    data_t1, data_t2, data_label, uc_position, c_position = get_trainSmp_Position(args.dataset)  # for GF5B
    ...
    x_train_t2, _ = get_train_and_test_data(mirror_image_t2, band, total_pos_train, total_pos_test,
                                                          patch=args.patch_size, band_patch=args.band_patches)
    y_train, y_test = train_and_test_label(number_train, number_test, num_classes) # for GF5B

    # -------------------------------------------------------------------------------
    # load data
    x_train_t1 = torch.from_numpy(x_train_t1.transpose(0,3,1,2)).type(torch.FloatTensor)  # [1000, 155, 5, 5]
    x_train_t2 = torch.from_numpy(x_train_t2.transpose(0,3,1,2)).type(torch.FloatTensor)  # [1000, 155, 5, 5]
...
def get_Testdata(args):
    ...
    _, x_test_t2 = get_train_and_test_data(mirror_image_t2, band, total_pos_train, total_pos_test,
                                                          patch=args.patch_size, band_patch=args.band_patches)
    _, y_test = train_and_test_label(number_train, number_test, num_classes) # for GF5B
    ...
    label_test_loader = Data.DataLoader(Label_test, batch_size=32, shuffle=False) # args.batch_size
...
def two_cls_access_for_Bay_Barbara(reference,result):
    ...
    # fp = set(label_1).intersection(set(predict_0))  # False Positive
    # fn = set(label_0).intersection(set(predict_1))  # False Negative
    fp = set(label_0).intersection(set(predict_1))  # False Positive
    fn = set(label_1).intersection(set(predict_0))  # False Negative

    # precision = len(tp) / len(label_1)
    # recall = len(tp) / len(predict_1)
